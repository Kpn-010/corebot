...
class Info(commands.Cog, name="Info"):
    ...
    async def info(self, ctx: commands.Context):
        ...
        await ctx.send("Info subcommands:\n"
    ...
    async def server(self, ctx: commands.Context):
        ...
        embed.add_field(
            name="Channels",
            value=f"{len(g.text_channels)} ·  {len(g.voice_channels)}")
        embed.add_field(name="Roles", value=str(len(g.roles)))
        embed.add_field(name="Emojis", value=str(len(g.emojis)))
        embed.add_field(name="Created",
                        value=discord.utils.format_dt(g.created_at, style="D"),
                        inline=False)
    ...
    async def user(self, ctx: commands.Context, *, target: str = None):
        ...
        roles = [
            r.mention for r in reversed(member.roles)
        ...
        color = member.color if member.color != discord.Color.default(
    ...
    async def role(self, ctx: commands.Context, *, target: str = None):
        ...
        if target:
            ...
            return await ctx.send(
                "✕ Please specify a role: `cc info role <@role|ID|name>`")

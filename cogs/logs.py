...
class Logs(commands.Cog, name="Logs"):
    ...
    async def _send(self, guild: discord.Guild, category: str, embed: discord.Embed) -> None:
    ...
    def _embed(self, title: str, color: discord.Color, **fields) -> discord.Embed:
    ...
    async def log(self, ctx: commands.Context):
        ...
        embed.set_footer(text="cc log set <category> <#channel>  |  cc log clear <category>")
        await ctx.send(embed=embed)

    @log.command(name="set")
    @commands.has_permissions(manage_guild=True)
    async def log_set(self, ctx: commands.Context, category: str, *, target: str):
        category = category.lower()
        if category not in LOG_CATEGORIES:
            return await ctx.send(f"✕ Unknown category. Choose: `{'` `'.join(LOG_CATEGORIES)}`")
    ...
    @commands.has_permissions(manage_guild=True)
    async def log_clear(self, ctx: commands.Context, category: str):
        category = category.lower()
        if category not in LOG_CATEGORIES:
            return await ctx.send(f"✕ Unknown category. Choose: `{'` `'.join(LOG_CATEGORIES)}`")
    ...
    async def log_mod(self, guild: discord.Guild, action: str, **fields) -> None:
        colors = {
        ...
        embed = self._embed(action, colors.get(action, discord.Color.blurple()), **fields)
    ...
    async def on_guild_role_create(self, role: discord.Role):
        embed = self._embed("Role Created", discord.Color.green(),
            Role=role.mention, Color=str(role.color), ID=f"`{role.id}`")
        await self._send(role.guild, "server", embed)

    @commands.Cog.listener()
    async def on_guild_role_delete(self, role: discord.Role):
        embed = self._embed("Role Deleted", discord.Color.red(),
            Role=role.name, Color=str(role.color), ID=f"`{role.id}`")
        await self._send(role.guild, "server", embed)
    ...
    async def on_guild_channel_create(self, channel: discord.abc.GuildChannel):
        embed = self._embed("Channel Created", discord.Color.green(),
            Channel=channel.mention, Type=type(channel).__name__, ID=f"`{channel.id}`")
        await self._send(channel.guild, "server", embed)

    @commands.Cog.listener()
    async def on_guild_channel_delete(self, channel: discord.abc.GuildChannel):
        embed = self._embed("Channel Deleted", discord.Color.red(),
            Channel=channel.name, Type=type(channel).__name__, ID=f"`{channel.id}`")
        await self._send(channel.guild, "server", embed)
    ...
    async def on_voice_state_update(
        ...
        if before.channel is None:
            ...
            embed = discord.Embed(title="Moved Voice", color=discord.Color.blurple())
            embed.set_author(name=str(member), icon_url=member.display_avatar.url)
            embed.add_field(name="User", value=member.mention, inline=True)
